...
jest.mock("../firebase/firestore/firestore", () => ({
  ...
  import { User, Landlord, Tenant, Residence, Apartment, MaintenanceRequest, LaundryMachine } from "@/types/types";
  import {
    createUser,
    getUser,
    updateUser,
  ...
  describe("Firestore Functions", () => {
    ...
    test("should update a landlord", async () => {
      ...
      expect(updateLandlord).toHaveBeenCalledWith(mockLandlord.userId, updatedData);
    });
    ...
    test("should update a residence", async () => {
      const updatedData = { tenants: ["tenant3"] };
      await updateResidence(mockResidence.residenceId, updatedData);
      expect(updateResidence).toHaveBeenCalledWith(mockResidence.residenceId, updatedData);
    });
    ...
    test("should update a maintenance request", async () => {
      ...
      expect(updateMaintenanceRequest).toHaveBeenCalledWith(mockRequest.requestID, updatedData);
    });
  
    test("should delete a maintenance request", async () => {
      await deleteMaintenanceRequest(mockRequest.requestID);
      expect(deleteMaintenanceRequest).toHaveBeenCalledWith(mockRequest.requestID);
    });
    ...
    test("should create a new laundry machine", async () => {
      ...
      expect(createLaundryMachine).toHaveBeenCalledWith(residenceId, mockLaundryMachine);
    });
    ...
    test("should retrieve a laundry machine", async () => {
      ...
      expect(getLaundryMachine).toHaveBeenCalledWith(residenceId, mockLaundryMachine.id);
    ...
    test("should update a laundry machine", async () => {
      ...
      expect(updateLaundryMachine).toHaveBeenCalledWith(residenceId, mockLaundryMachine.id, updatedData);
    });
  
    test("should delete a laundry machine", async () => {
      await deleteLaundryMachine(residenceId, mockLaundryMachine.id);
      expect(deleteLaundryMachine).toHaveBeenCalledWith(residenceId, mockLaundryMachine.id);
    });
  });
