...
describe("LandlordListIssuesScreen", () => {
  ...
  test("toggles filter section visibility", async () => {
    const screen = render(<LandlordListIssuesScreen />);

    // Wait for the initial asynchronous state updates to complete
    await waitFor(() => {
      // Wait for an element that appears after the data is loaded
      expect(screen.getByText("Maintenance issues")).toBeTruthy();
    });
